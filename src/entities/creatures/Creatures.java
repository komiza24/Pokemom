    ...
    public void moveX() {
        ...
        if (xMove > 0) {  //right
            ...
            if (!collisionWithTile(tx, (int) (y /** Tile.TILEHEIGHT */+ bounds.y) / Tile.TILEHEIGHT)
                    && // oben rechts
                    !collisionWithTile(tx, (int) (y /** Tile.TILEHEIGHT*/ + bounds.y + bounds.height) / Tile.TILEHEIGHT)) {  // unten rechts
                x += xMove;
            }else{
            ...
            if (!collisionWithTile(tx, (int) (y /** Tile.TILEHEIGHT*/ + bounds.y) / Tile.TILEHEIGHT)
                    && // oben links
                    !collisionWithTile(tx, (int) (y /** Tile.TILEHEIGHT*/ + bounds.y  + bounds.height) / Tile.TILEHEIGHT)) {  // unten links
                x += xMove;
            }else{ 
    ...
    public void moveY() {
        ...
        if (yMove < 0) {//up
            ...
            if (!collisionWithTile((int) (x /** Tile.TILEWIDTH */+ bounds.x) / Tile.TILEWIDTH, ty)
                    &&// oben links
                    !collisionWithTile((int) (x /** Tile.TILEWIDTH*/ + bounds.x + bounds.width) / Tile.TILEWIDTH, ty)) {   // oben rechts
                y += yMove;
